...
export const Route = createFileRoute("/panel/solicitantes")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Registro y consulta de solicitantes para evaluación crediticia.",
      },
      { property: "og:title", content: "Solicitantes — Veridica" },
      {
...
const emptyForm: ApplicantInput = {
  nombreCompleto: "",
  numeroDocumento: "",
  ingresosMensuales: 0,
  deudasMensuales: 0,
...
function ApplicantsPage() {
  ...
  function update<K extends keyof ApplicantInput>(key: K, value: ApplicantInput[K]) {
    ...
    try {
      ...
      setNotice(
        isApiConfigured
          ...
          : "Validación completada. Conecta la API para guardar el registro.",
      );
      setForm(emptyForm);
      setOpen(false);
    } catch (error) {
      setNotice(
        error instanceof ApiRequestError
          ...
          : "No fue posible registrar el solicitante.",
      );
    } finally {
      setLoading(false);
    }
  ...
  async function search(event: FormEvent) {
    ...
    const valid = z
      ...
      .safeParse(document);
    if (!valid.success) {
      setNotice("El documento debe contener entre 6 y 15 dígitos.");
      return;
    }
    setLoading(true);
    try {
      if (isApiConfigured) setResult(await api.findApplicant(document));
        ...
        setResult({
          ...
          nombreCompleto: found.name,
          numeroDocumento: found.document,
          ingresosMensuales: found.income,
          deudasMensuales: found.debt,
          numeroMoras: found.arrears,
          historialCrediticio: found.history,
          antiguedadLaboral: 5,
        });
      }
    } catch (error) {
      ...
      setNotice(
        error instanceof ApiRequestError && error.status === 404
          ? "No encontramos un solicitante con ese documento."
  ...
  return (
    <>
      <PageHeader
        ...
        title="Solicitantes"
        description="Registra la información financiera y consulta perfiles existentes antes de iniciar una evaluación."
        action={
      ...
      <Panel className="p-5 sm:p-6">
        <form onSubmit={search} className="grid gap-3 sm:grid-cols-[minmax(0,1fr)_auto]">
          <Field
            ...
            htmlFor="documento"
            hint="Entre 6 y 15 dígitos, sin puntos ni espacios."
          >
            <div className="relative">
              ...
              <input
                ...
                value={document}
                onChange={(event) =>
                  setDocument(event.target.value.replace(/\D/g, "").slice(0, 15))
                }
          ...
          <Button type="submit" className="self-start sm:mt-[1.65rem]" disabled={loading}>
        ...
        {result && (
          <div className="mt-6 border-t border-border pt-5">
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
              <Data label="Solicitante" value={result.nombreCompleto} />
              <Data label="Documento" value={result.numeroDocumento} />
              <Data label="Ingresos" value={currency(result.ingresosMensuales)} />
              <Data label="Deudas" value={currency(result.deudasMensuales)} />
              <Data label="Moras" value={String(result.numeroMoras)} />
              <Data label="Historial" value={result.historialCrediticio} />
              <Data label="Antigüedad laboral" value={`${result.antiguedadLaboral} años`} />
              <Data
                label="Registro"
                value={new Date(result.fechaRegistro).toLocaleDateString("es-CO")}
              />
            </div>
          </div>
        )}
      ...
      {open && (
        <div className="fixed inset-0 z-50 grid place-items-center bg-overlay p-4">
          <div
            ...
            className="max-h-[92vh] w-full max-w-3xl overflow-y-auto rounded-xl border border-border bg-surface shadow-xl"
          >
            <div className="grid grid-cols-[minmax(0,1fr)_auto] items-center border-b border-border px-5 py-4">
              <div>
                <h2 id="new-title" className="font-display text-xl font-bold">
                  Nuevo solicitante
                </h2>
            ...
            <form onSubmit={submit} className="grid gap-5 p-5 sm:grid-cols-2">
              <Field label="Nombre completo" htmlFor="nombre" error={errors["nombreCompleto"]}>
                <input
                  id="nombre"
              ...
              <Field
                ...
                htmlFor="historial"
                error={errors["historialCrediticio"]}
              >
                <select
                  id="historial"
                  className="form-control"
              ...
              <Field
                ...
                htmlFor="antiguedad"
                error={errors["antiguedadLaboral"]}
              >
                <input
                  id="antiguedad"
                  className="form-control"
